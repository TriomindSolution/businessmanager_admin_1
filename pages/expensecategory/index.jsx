...
import { EXPENSECATEGORY_END_POINT } from "@/constants/api_endpoints/expensecategoryEndPoints";
...
const DeleteModal = ({ isOpen, onClose, data, isParentRender }) => {
    ...
    const deleteData = async () => {
        try {
            const response = await http.delete(EXPENSECATEGORY_END_POINT.delete(data?.id));
...
const Expensecategory = () => {
    ...
    /*** Pagination Start  */
    const pagination = {
        total: expenseCategoryList?.length,
        current: page,
        pageSize: perPage,
        defaultPageSize: 10,
...
export default withAuth(Expensecategory)